...
<b>format() :</b> " . $sql->format($s, $sd) . "<hr>";
                ...
                $sd = $sql->getData();
                $echo[] = "<pre>\$sql->select('*', 'order')->where([ 'a' => 1, 'b' => 2 ])->unionAll(\$sql->copy('abc'));</pre>
<b>getSql() :</b> {$s}<br>
<b>getData():</b> <pre>" . json_encode($sd, JSON_PRETTY_PRINT) . "</pre>
<b>format() :</b> " . $sql->format($s, $sd) . "<hr>";
                ...
                $s = $sql->getSql();
                $sd = $sql->getData();
                $echo[] = "<pre>\$sql->copy('abcd', [ 'where' => [ 'c' => 2 ] ]);</pre>
<b>getSql() :</b> {$s}<br>
<b>getData():</b> <pre>" . json_encode($sd, JSON_PRETTY_PRINT) . "</pre>
<b>format() :</b> " . $sql->format($s, $sd);
                break;
...
<b>format() :</b> " . $sql->format($s, $sd);
                ...
                $s = $sql->update('json', [ 'json1' => [ 'key' => 'val', 'key2' => 'val2' ], 'json2' => [ [ 'k1' => 'v1' ], [ 'k2' => 'v2' ] ], 'json3' => [ 'x' => 1, 'y' => 2 ], 'json4' => [] ])->where([ 'id' => 1 ])->getSql();
                $sd = $sql->getData();
                $echo[] = "<pre>\$sql->update('json', [ 'json1' => [ 'key' => 'val', 'key2' => 'val2' ], 'json2' => [ [ 'k1' => 'v1' ], [ 'k2' => 'v2' ] ], 'json3' => [ 'x' => 1, 'y' => 2 ], 'json4' => [] ])->where([ 'id' => 1 ]);</pre>
<b>getSql() :</b> {$s}<br>
<b>getData():</b> <pre>" . json_encode($sd, JSON_PRETTY_PRINT) . "</pre>
<b>format() :</b> " . $sql->format($s, $sd);
